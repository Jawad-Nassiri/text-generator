...
btn.addEventListener('click', function(event){
  ...
  if(inputValue < 0 || inputValue > 9 || inputValue === '' || isNaN(inputValue)){
    resElement.innerHTML = dummyText[randomText];
  }else{
    let sliceArray = dummyText.slice(0, inputValue);

    let finalText = sliceArray.map(function(item){
      return "<p>"+ item +"</p>";
    }).join('');

    resElement.innerHTML = finalText;
  }
